...
try {
    ...
    # Does this Excel have the dynamic-array engine? Probe with a NATIVE DA
    # function, not one of ours — on a pre-DA build (and this ecosystem is still
    # tested on such builds) a multi-cell return paints only its top-left cell,
    # so a plain-formula spill assertion would be vacuous rather than failing
    # loudly. When DA is absent we enter the grid the documented pre-DA way:
    # legacy CSE over a pre-selected range (see xll.yaml's Spill section).
    $ws.Range("Z1").Formula = '=SEQUENCE(2,2)'
    Start-Sleep 1
    $hasDA = $null -ne $ws.Range("AA1").Value2
    $ws.Range("Z1:AA2").ClearContents()
    Write-Output "dynamic arrays: $(if ($hasDA) { 'available' } else { 'NOT available (pre-DA build) — using legacy CSE for the grid check' })"

    # --- 1. the error reaches the cell -------------------------------------
    Write-Output "[1] grid rtd-once handler error must PAINT, not wedge"
    $ws.Range("A1").Formula = '=YDH("MSFT",0)'
    ...
    Start-Sleep 2
    $ws.Range("A10").Formula = '=YDH("MSFT",0)'
    ...
    if ($hasDA) {
        $ws.Range("C1").Formula = '=YDH("MSFT",30)'
    } else {
